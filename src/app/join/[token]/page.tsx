...
export default function JoinPage() {
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(
            payload.error || t('conflictDescription', { name: peek.account_name }),
          );
        } else {
          toast.error(payload.error || t('failedToAccept'));
        }
        setAccepting(false);
        return;
      }
      toast.success(t('welcomeToTheTeam'));
  ...
  // ----- Peek failed -----
  if (!peek.ok) {
    const failTitleKey = peek.reason === 'not_found' ? 'notFoundTitle' : peek.reason === 'used' ? 'usedTitle' : peek.reason === 'expired' ? 'expiredTitle' : 'serverErrorTitle';
    const failBodyKey = peek.reason === 'not_found' ? 'notFoundBody' : peek.reason === 'used' ? 'usedBody' : peek.reason === 'expired' ? 'expiredBody' : 'serverErrorBody';
    return (
      <Card className="w-full max-w-md border-border bg-card">
        <CardHeader className="items-center text-center">
          ...
          <CardTitle className="text-xl text-foreground">{t(failTitleKey)}</CardTitle>
  ...
  const inviteHeader = (
    <CardHeader className="items-center text-center">
      ...
      <CardTitle className="text-xl text-foreground">
        {t('youAreInvitedTo', { name: peek.account_name })}
      </CardTitle>
      <CardDescription className="text-muted-foreground">
        {t('youWillJoinAs', { role: rolesT(peek.role) })}
        . {t('linkValidUntil', { date: new Date(peek.expires_at).toLocaleDateString(undefined, {
          year: 'numeric',
          month: 'short',
          day: 'numeric',
        }) })}
        ...
        <Card className="w-full max-w-md border-border bg-card">
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            <p className="text-center text-xs text-muted-foreground">
              {t('acceptingDescription', { name: peek.account_name })}
            </p>
          </CardContent>
        </Card>
        ...
        >
          <DialogContent className="bg-popover border-border sm:max-w-md">
            ...
            <div className="space-y-2 py-2 text-xs text-muted-foreground">
              <p>
                {t('conflictDescription', { name: peek.account_name })}
              </p>
            </div>
