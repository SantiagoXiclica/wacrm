...
export function PipelineDonut({ data, loading, currency }: PipelineDonutProps) {
  const t = useTranslations('pipelineDonut')
  return (
    <section className="flex h-full flex-col rounded-xl border border-border bg-card">
      ...
      <div className="flex flex-1 flex-col p-5">
        {loading || !data ? (
          ...
          <>
            <Donut data={data} currency={currency} ariaLabel={t('ariaLabel')} totalLabel={t('total')} />
...
// ------------------------------------------------------------
function Donut({ data, currency, ariaLabel, totalLabel }: { data: PipelineDonutData; currency: string; ariaLabel: string; totalLabel: string }) {
  const size = 200
  const r = 80
  const ringWidth = 18
  const cx = size / 2
