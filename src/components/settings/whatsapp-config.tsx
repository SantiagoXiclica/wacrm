  ...
  async function handleSave() {
    ...
    try {
      ...
      if (data.registered === false && data.registration_error) {
        toast.error(
          t('savedButNotRegistered', { error: data.registration_error }),
          { duration: 12000 },
        );
        ...
        toast.success(
          data.phone_info?.verified_name
            ? t('liveStatus', { name: data.phone_info.verified_name })
            : t('connectedMessage'),
        );
  ...
  async function handleTestConnection() {
    try {
      ...
      if (payload.connected) {
        ...
        setResetReason(payload.needs_reset ? 'token_corrupted' : payload.reason === 'meta_api_error' ? 'meta_api_error' : null);
        setStatusMessage(payload.message || '');
        toast.error(payload.message || t('apiTestFailed'));
      }
    } catch (err) {
      console.error('Test connection error:', err);
      setConnectionStatus('disconnected');
      toast.error(t('apiTestNetworkError'));
    } finally {
      setTesting(false);
    }
  }
          ...
          >
            ...
            <AlertDescription className="text-muted-foreground mt-2 text-xs leading-relaxed">
              {isRegistered ? (
                ...
                <span>{t('registrationSkipped')}</span>
              )}
            </AlertDescription>
            ...
            {registrationProbe && (
              <div className="mt-3 rounded border border-border bg-card/60 px-3 py-2 space-y-1.5 text-[11px]">
                <p className="font-medium text-foreground">
                  {t('diagnostic', { status: registrationProbe.live ? 'live' : 'not live' })}
                </p>
