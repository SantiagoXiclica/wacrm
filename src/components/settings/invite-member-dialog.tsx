...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our NEXIA CRM account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    const accountName = result?.accountName ?? 'our NEXIA CRM account';
    const message = `Join ${accountName} on NEXIA CRM using this link (valid for ${result?.expiresInDays} days): ${url}`;
