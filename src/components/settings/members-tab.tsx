...
function fmtExpiresIn(iso: string, t?: ReturnType<typeof useTranslations>): string {
  ...
  if (ms <= 0) return t ? t('expired') : 'expired';
  const days = Math.ceil(ms / (24 * 60 * 60 * 1000));
  return t ? t('expiresIn', { n: days }) : `expires in ${days} day${days === 1 ? '' : 's'}`;
}
...
export function MembersTab() {
  ...
  const loadEverything = useCallback(async () => {
    try {
      ...
      if (!mres.ok) {
        ...
        toast.error(payload.error || t('failedToLoad'));
        return;
      }
      ...
      if (ires) {
        if (!ires.ok) {
          ...
          toast.error(payload.error || t('failedToLoadInvites'));
          return;
        }
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || t('failedToUpdateRole'));
        return;
      }
      toast.success(t('updatedRole', { name: member.full_name || t('unnamed'), role: nextRole }));
    } catch (err) {
  ...
  async function handleRemove() {
    ...
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('failedToRemove'));
        return;
      }
      toast.success(t('removed', { name: removingMember.full_name || t('unnamed') }));
      setMembers((prev) =>
  ...
  async function handleRevoke(invite: Invitation) {
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('failedToRevoke'));
        return;
      }
          ...
          return (
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
              <span className="inline-flex items-center gap-1.5">
                ...
                {t('online', { n: counts.online })}
              </span>
              <span className="inline-flex items-center gap-1.5">
                <PresenceDot status="away" />
                {t('away', { n: counts.away })}
              </span>
              <span className="inline-flex items-center gap-1.5">
                        ...
                        <SelectContent>
                          {EDITABLE_ROLES.map((r) => (
                            <SelectItem key={r.value} value={r.value}>
                              {t(`role${r.value.charAt(0).toUpperCase() + r.value.slice(1)}`)}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-0.5 text-xs text-muted-foreground">
                          {t('createdDate', { date: fmtDate(inv.created_at) })} · {fmtExpiresIn(inv.expires_at, t)}
                        </p>
                      </div>
        ...
        <DialogContent className="bg-popover border-border sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              {t('removeBody', { name: removingMember?.full_name || t('unnamed') })}
            </DialogDescription>
          </DialogHeader>
