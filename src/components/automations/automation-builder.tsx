...
const getStepMeta = (t: ReturnType<typeof useTranslations<'automationBuilder'>>): Record<AutomationStepType, StepMeta> => ({
  send_message: { label: t('messageText'), icon: MessageSquare, border: "border-l-primary" },
  send_template: { label: t('triggerType'), icon: FileText, border: "border-l-primary" },
  add_tag: { label: t('addTag'), icon: Tag, border: "border-l-primary" },
  remove_tag: { label: t('removeTag'), icon: TagIcon, border: "border-l-primary" },
  assign_conversation: { label: t('triggerConversationAssigned'), icon: UserCheck, border: "border-l-primary" },
  update_contact_field: { label: t('contactField'), icon: PencilLine, border: "border-l-primary" },
  create_deal: { label: t('createDeal'), icon: Briefcase, border: "border-l-primary" },
  wait: { label: t('triggerTimeBased'), icon: Hourglass, border: "border-l-border" },
  condition: { label: t('triggerKeyword'), icon: GitBranch, border: "border-l-amber-500" },
  send_webhook: { label: t('url'), icon: Webhook, border: "border-l-primary" },
  close_conversation: { label: t('closeConversation'), icon: CircleSlash, border: "border-l-primary" },
})
...
const getTriggerOptions = (t: ReturnType<typeof useTranslations<'automationBuilder'>>): { value: AutomationTriggerType; label: string; hint: string }[] => [
  { value: "new_message_received", label: t('triggerNewMessage'), hint: t('triggerFirstMessage') },
  { value: "first_inbound_message", label: t('triggerFirstMessage'), hint: t('triggerFirstMessage') },
  { value: "keyword_match", label: t('triggerKeyword'), hint: t('keywordPlaceholder') },
  { value: "new_contact_created", label: t('triggerNewContact'), hint: t('keywordPlaceholder') },
  { value: "conversation_assigned", label: t('triggerConversationAssigned'), hint: t('keywordPlaceholder') },
  { value: "tag_added", label: t('triggerTagAdded'), hint: t('keywordPlaceholder') },
  { value: "time_based", label: t('triggerTimeBased'), hint: t('keywordPlaceholder') },
]
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="truncate text-sm font-medium text-foreground">
              {getTriggerOptions(t).find((o) => o.value === type)?.label ?? type}
            </div>
          </div>
          <ChevronDown
          ...
          <div className="space-y-3 border-t border-border px-4 py-3">
            <div>
              ...
              <p className="mt-1 text-[11px] text-muted-foreground">
                {getTriggerOptions(t).find((o) => o.value === type)?.hint}
              </p>
            </div>
            {type === "keyword_match" && (
              <KeywordMatchConfig
...
function StepRenderer({
  ...
  const meta = getStepMeta(t)[step.step_type]
      ...
      <DropdownMenu>
        ...
        >
          {ADDABLE_STEPS.map((stepType) => {
            const Icon = getStepMeta(t)[stepType].icon
            return (
              <DropdownMenuItem key={stepType} onClick={() => onPick(stepType)}>
                <Icon className="h-4 w-4" />
                {getStepMeta(t)[stepType].label}
              </DropdownMenuItem>
            )
          })}
        </DropdownMenuContent>
